...
const BOOTCAMP_COURSES = [
  ...
  {
    id: "c_jour2",
    title: "Jour 2. Programmation Python",
    summary: "Python parle, décide, et écrit le texte de ta vitrine.",
...
const BOOTCAMP_ASSIGNMENTS = [
  ...
  },
  {
    id: "a_bio_python",
    courseId: "c_jour2",
    title: "Mini exo : un script écrit ta bio",
    brief: "Dépose ton fichier bio.py : il demande ton prénom, une phrase, 3 envies, et affiche le texte de ta vitrine.",
    dueAt: "",
  },
];
...
const seedBootcamp = (data) => {
  ...
  for (const course of BOOTCAMP_COURSES) {
    if (!data.courses.some((item) => item.id === course.id)) {
      data.courses.push({ ...course, createdAt: Date.now() });
    }
  }
  for (const work of BOOTCAMP_ASSIGNMENTS) {
    if (!data.assignments.some((item) => item.id === work.id)) {
